...
public class MongoDBImportCollegeDataIAndCreateIndex {
    private static String IP = "localhost";
    private static int PORT = 27017;
    private static String DB_NAME = "designyourcollege";
    private static String COLLECTION_NAME = "colleges";
...
//                        doc.append(filedNames[i], values[i]);
            ...
            db.getCollection(COLLECTION_NAME).createIndex(Indexes.geo2dsphere("loc"));
            db.getCollection(COLLECTION_NAME).createIndex(Indexes.text("name"));
            IndexOptions indexOptions = new IndexOptions().unique(true);
            db.getCollection(COLLECTION_NAME).createIndex( new Document("id",1), indexOptions);
            db.getCollection(COLLECTION_NAME).createIndex( new Document("state",1));
            db.getCollection(COLLECTION_NAME).createIndex( new Document("control",1));
            db.getCollection(COLLECTION_NAME).createIndex( new Document("urbanization",1));
            db.getCollection(COLLECTION_NAME).createIndex( new Document("religious_affiliation",1));
            db.getCollection(COLLECTION_NAME).createIndex( new Document("offers_associate_degree",1).append("offers_bachelor_degree",1).append("offers_master_degree",1)
                   .append("offers_doctor_degree_research_scholarship",1).append("offers_doctor_degree_professional_practice",1));
            db.getCollection(COLLECTION_NAME).createIndex( new Document("tuition_and_fees",1));
